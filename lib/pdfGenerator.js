...
export const generatePDF = async (resumeData, templateId) => {
  return new Promise(async (resolve, reject) => {
    try {
      console.log('Starting PDF generation with clickable links...');
      ...
      if (window.__resumeTemplateElement) {
        ...
        console.log('✅ Using global template ref');
      }
      
      // Method 2: Find by template ID
      if (!template) {
        template = document.querySelector(`[data-template="${templateId}"]`);
        if (template) console.log('✅ Found template by data-template');
      }
      ...
      if (!template) {
        ...
        if (previewContainer) {
          ...
          if (template) console.log('✅ Found template in preview container');
        }
      }
      ...
      const extractLinks = (element, parentPath = '') => {
        ...
        anchorTags.forEach((link, index) => {
          ...
          console.log(`Found link: ${text} -> ${url} at (${rect.left}, ${rect.top})`);
        });
      };
      
      // Extract all links from the template
      extractLinks(template);
      console.log(`Extracted ${links.length} links from template`);
      ...
      console.log('Creating PDF with clickable annotations...');
      ...
      links.forEach(link => {
        ...
        console.log(`Adding link: ${link.text} -> ${link.href} at (${x}, ${y}) size (${width}, ${height})`);
...
export const debugLinks = () => {
  ...
  anchorTags.forEach((link, index) => {
    const href = link.getAttribute('href');
    const rect = link.getBoundingClientRouterect();
    const text = link.textContent.trim();
    const icon = link.querySelector('[style*="contactIcon"]')?.textContent || '';
